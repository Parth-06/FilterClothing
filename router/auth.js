...
router.get ('/home', authenticate, async (req,res)=> {
  ...
  router.post ('/productsdata', authenticate, async (req,res)=> {
    ...
    try{
      const listOne = await Productlist.findOne({_id : id})
      if(listOne){
        const userNew =  await Productlist.findByIdAndUpdate({_id: id},{
          $push:
          { clothes: user}    
          });
      console.log(userNew);
  ...
  router.post ('/cartdata', authenticate, async (req,res)=> {
    ...
    if(listOnee){
      try{
    if (g === 0 ){
      const userNew =  await Cartlist.findByIdAndUpdate({_id: listOnee.id},{
        $set:
        {cart: newcartvalue}    
        });
    }else{
...
console.log(err)
   ...
   if(listtwo){
      try{
        const userNeww =  await Cartlist.findOneAndUpdate({email: req.rootUser.email ,'cart.id' : prod_id}, 
        {
         'cart.$.qty': quantity
        });
  ...
  router.post ('/deletedata', authenticate, async (req,res)=> {
    ...
    if(listtwo){
        try{
                ...
                const userNew =  await Cartlist.findOneAndUpdate({'cart.id' : prodid, email :  req.rootUser.email }, 
            {$pull:{
              cart:{id: prodid}
            }}
               );
